...
func request(url: String, include_github_api: bool = true, download_to_file: bool=false, download_path: String="user://") -> Dictionary:
	# Headers
	var headers := []
	...
	if result[0] != OK or result[1] != 200:
		print("Failed, result: %s, response_code: %s" % [result[0], result[1]])
